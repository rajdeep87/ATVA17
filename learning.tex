...
\begin{definition}
  ...
  $\domain$, \rmcmt{$\conf{\formula}{\domain}(a) = \{u \in \domain \mid u \in decomp(a) 
  \vee u \not\models \formula \}$}, \pscmt{TODO: I do not understand what the
  domain of $u$ is. Also, it is unclear what the co-domain of $\conf{}{}$
  is.}that is, it returns the set of abstract models that do not satisfy 
...
abstract element \rmcmt{$a=(y\leq-5)$, $\conf{\formula}{\intervals}(a) = \{(y\leq-5), 
(y \leq -4 \land x \leq -3), (y \leq -3 \land x \leq -2), (y \leq -2 \land x <
0)\}$}.  
\pscmt{TODO: I do not understand the notation (..,..). Please clarify.}.
...
is given by, \rmcmt{$\abd{\formula}{\intervals}(y \leq -5)=(y \leq -2 \land x <
0)$} \pscmt{TODO: again, the notation is mysterious},
which clearly underapproximates $\conf{\formula}{\intervals}$ as well as strictly
generalizes the reason for $y \leq -5$. 
%


The main idea of abductive  reasoning is to iteratively replace \rmcmt{an abstract
element} $s$ \pscmt{TODO: Have you defined this terms in the context of domains? I don't understand the word ``assignment'' in the context of a relational domain.} in the conflict reason by a partial assignment that is sufficient 
