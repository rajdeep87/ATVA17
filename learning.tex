...
formula.  
\begin{definition}
  Given a formula $\formula$, an initial abstract value $a$ and domain 
  $\domain$, $\conf{\formula}{\domain}(a) = \{u \in \domain \mid u \in a 
  \vee u \not\models \formula \}$, that is, it returns the set of abstract models 
  that do not satisfy $\formula$ or are approximated by $a$.
\end{definition}
%
...
0)\}$.  Informally, $\conf{\formula}{\intervals}$  computes the most general set of
...
is given by, $\abd{\formula}{\intervals}(y \leq -5)=(y \leq -2 \land x <
0)$, which clearly underapproximates $\conf{\formula}{\intervals}$ as well as 
