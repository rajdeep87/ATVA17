...
\rmcmt{
\begin{definition}
  Given formula $\formula$, an initial abstract value $\allval$ and domain 
  $\domain$, $\abd{\formula}{\domain}(A) = \{u \mid u \in A \vee u \not\models
  \formula \}$, that is, it adds abstract models to the input set $\allval$, that do
  not satisfy $\formula$.
\end{definition}
}
%
