...
decision increases the decision level by one. Given 
the current abstract value $\abs(\trail)$, the procedure $\decide$ 
in Algorithm~\ref{Alg:ms} heuristically returns a meet irreducible.
...
and $d$ is a constant.
%\in \mathbb{R}\cup\{\infty\}$ is the bound of the inequality \pscmt{we have bitvectors!}. 
%
